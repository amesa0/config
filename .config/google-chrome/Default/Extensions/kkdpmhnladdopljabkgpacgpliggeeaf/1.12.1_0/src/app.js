(function() {
  ...
  require(this.ledger.imports, function() {
    ...
    Application = (function(_super) {
      ...
      Application.prototype.onDongleIsUnlocked = function(dongle, skipFirmCheck) {
        if (!this.isInWalletMode()) {
          return;
        }
        if (ledger.app.dongle.isNanoS() && ledger.app.dongle.getIntFirmwareVersion() < 805372417 && !skipFirmCheck) {
          return ledger.app.router.go('/onboarding/device/firmware');
        } else {
                    ...
                    return ledger.app.dongle.getPublicAddress("44'/" + networks[0].bip44_coin_type + "'/0'/0/0", function(addr) {
                      ...
                      return ledger.storage.global.chainSelector.get(address, function(result) {
                        ...
                        if (result[address] != null) {
                          ...
                          if (exists) {
                            _this.onChainChosen(ledger.bitcoin.Networks[exists]);
                          }
                        }
                        if ((result[address] == null) || !exists) {
    ...
    this.FIRMWARE_LAYOUT = 'UpdateNavigationController';
