  ...
  _.extend(ledger.bitcoin, {
    ...
    estimateTransactionSize: function(inputsCount, outputsCount) {
      ...
      if (ledger.config.network.handleSegwit) {
        minNoWitness = varintLength + 4 + 2 + (59 * inputsCount) + 1 + (32 * outputsCount) + 4;
        maxNoWitness = varintLength + 4 + 2 + (59 * inputsCount) + 1 + (34 * outputsCount) + 4;
        minWitness = varintLength + 4 + 2 + (59 * inputsCount) + 1 + (32 * outputsCount) + 4 + (106 * inputsCount);
        maxWitness = varintLength + 4 + 2 + (59 * inputsCount) + 1 + (34 * outputsCount) + 4 + (108 * inputsCount);
