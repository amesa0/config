(function() {
  ...
  this.ledger.dongle.Dongle = (function(_super) {
    ...
    Dongle.prototype.isNanoS = function() {
      return this.productId === 0x0001;
    };
    ...
    Dongle.prototype.ensureDeviceIsUnlocked = function(runForever) {
      ...
      return this._ensureDeviceIsUnlockedTimeout = _.delay((function(_this) {
        return function() {
          return _this.getPublicAddress("0'/0xb11e'").then(function() {
            return true;
          }).fail(function(err) {
    ...
    Dongle.prototype._createPaymentTransactionNew = function(inputs, associatedKeysets, changePath, changeAddress, recipientAddress, amount, fees, lockTime, sighashType, authorization, data, resumeData) {
      return this.getPublicAddress(changePath).then((function(_this) {
        return function(result) {
          var AdjustAlign, OP_CHECKBLOCKATHEIGHT, OP_CHECKSIG, OP_DUP, OP_EQUAL, OP_EQUALVERIFY, OP_HASH160, OP_RETURN, OpReturnScript, P2shScript, ParamHash, ParamHeight, PkScript, VI, ZenReplayProtection, changeAddress2, changeAmount, inputAmount, inputAmounts, numberOfOutputs, outputScript, task, totalInputAmount, _i, _len;
          ...
          if (ledger.config.network.name === "zencash") {
          ...
          PkScript = function(address) {
            ...
            if (ledger.config.network.name === "zencash") {
              script = OP_DUP.concat(OP_HASH160).concat(new ByteString(Convert.toHexByte(hash160.length), HEX)).concat(hash160).concat(OP_EQUALVERIFY).concat(OP_CHECKSIG).concat(ZenReplayProtection);
          ...
          P2shScript = function(hash160) {
            var script;
            if (ledger.config.network.name === "zencash") {
              script = OP_HASH160.concat(new ByteString(Convert.toHexByte(hash160.length), HEX)).concat(hash160).concat(OP_EQUAL).concat(ZenReplayProtection);
              return VI(script.length).concat(script);
            } else {
          ...
          OpReturnScript = function(data) {
            var script;
            if (ledger.config.network.name === "zencash") {
              script = OP_RETURN.concat(new ByteString(Convert.toHexByte(data.length / 2), HEX)).concat(new ByteString(data, HEX)).concat(ZenReplayProtection);
              return VI(script.length).concat(script);
            } else {
